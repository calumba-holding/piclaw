...
<body>${X}</body>
</html>`}function d8({widget:_,onClose:$,onWidgetEvent:j}){let Z=k(null),N=k(!1),G=R_(()=>n8(_),[_]);if(S(()=>{if(!_)return;let A=(M)=>{if(M.key==="Escape")$?.()};return document.addEventListener("keydown",A),()=>document.removeEventListener("keydown",A)},[_,$]),S(()=>{N.current=!1},[G]),S(()=>{if(!_)return;let A=Z.current;if(!A)return;let M=(y)=>{try{A.contentWindow?.postMessage({__piclawGeneratedWidgetHost:!0,type:y,widgetId:_?.widgetId||null,toolCallId:_?.toolCallId||null,turnId:_?.turnId||null,payload:y==="widget.init"?M1(_):C4(_)},"*")}catch{}},c=()=>{N.current=!0,M("widget.init"),M("widget.update")};return A.addEventListener("load",c),()=>A.removeEventListener("load",c)},[_,G]),S(()=>{if(!_||!N.current)return;let A=Z.current;if(!A?.contentWindow)return;try{A.contentWindow.postMessage({__piclawGeneratedWidgetHost:!0,type:"widget.update",widgetId:_?.widgetId||null,toolCallId:_?.toolCallId||null,turnId:_?.turnId||null,payload:C4(_)},"*")}catch{}return},[_]),S(()=>{if(!_)return;let A=(M)=>{let c=Z.current;if(!c?.contentWindow||M.source!==c.contentWindow)return;let y=M?.data;if(!y||y.__piclawGeneratedWidget!==!0)return;let R=g0({widgetId:y.widgetId,toolCallId:y.toolCallId}),n=g0(_);if(R&&n&&R!==n)return;j?.(y,_)};return window.addEventListener("message",A),()=>window.removeEventListener("message",A)},[_,j]),!_)return null;let q=(_?.artifact||{}).kind||_?.kind||"html",L=typeof _?.title==="string"&&_.title.trim()?_.title.trim():"Generated widget",X=typeof _?.subtitle==="string"&&_.subtitle.trim()?_.subtitle.trim():"",Q=_?.source==="live"?"live":"timeline",V=typeof _?.status==="string"&&_.status.trim()?_.status.trim():"final",H=Q==="live"?`Live widget • ${V.toUpperCase()}`:_?.originPostId?`Message #${_.originPostId}`:"Timeline launch",P=typeof _?.description==="string"&&_.description.trim()?_.description.trim():"",f=!G,m=l8(_),g=c8(_);return B`
...
`).reduce((h_,w_)=>h_+f(w_),0):0,c_=Math.max(p_-o_,0);return{text:a_,omitted:c_,totalLines:p_,visibleLines:o_}},g=V(j),A=V(Z),M=V($),c=Boolean(g.text)||g.totalLines>0,y=Boolean(A.text)||A.totalLines>0,R=Boolean(M.fullText?.trim()||M.text?.trim());if(!_&&!R&&!c&&!y&&!N&&!G)return null;let[n,e]=v(new Set),[o,j_]=v(()=>Date.now()),i=($_)=>e((F_)=>{let Z_=new Set(F_),H_=!Z_.has($_);if(H_)Z_.add($_);else Z_.delete($_);if(typeof L==="function")L($_,H_);return Z_});S(()=>{e(new Set)},[Y]);let X_=t2(_);S(()=>{if(!X_)return;j_(Date.now());let $_=setInterval(()=>j_(Date.now()),1000);return()=>clearInterval($_)},[X_,_?.started_at,_?.startedAt]);let d_=_?.turn_id||Y,g_=a8(d_),d=q?"turn-dot turn-dot-queued":"turn-dot",N_=($_)=>$_,__=Boolean(_?.last_activity||_?.lastActivity),V_=($_)=>$_==="warning"?"#f59e0b":$_==="error"?"var(--danger-color)":$_==="success"?"var(--success-color)":g_,Y_=G?.kind||"info",L_=V_(Y_),B_=V_(_?.kind||(X_?"warning":"info")),G_="",u_=_?.title,I_=_?.status;if(_?.type==="plan")G_=u_?`Planning: ${u_}`:"Planning...";else if(_?.type==="tool_call")G_=u_?`Running: ${u_}`:"Running tool...";else if(_?.type==="tool_status")G_=u_?`${u_}: ${I_||"Working..."}`:I_||"Working...";else if(_?.type==="error")G_=u_||"Agent error";else G_=u_||I_||"Working...";if(__)G_="Last activity just now";let y_=({panelTitle:$_,text:F_,fullText:Z_,totalLines:H_,maxLines:M_,titleClass:a_,panelKey:p_})=>{let o_=n.has(p_),c_=Z_||F_||"",h_=p_==="thought"||p_==="draft"?P(c_):c_,w_=typeof M_==="number",k_=o_&&w_,m_=w_?m(h_,M_,H_):{text:h_||"",omitted:0,totalLines:Number.isFinite(H_)?H_:0};if(!h_&&!(Number.isFinite(m_.totalLines)&&m_.totalLines>0))return null;let W0=`agent-thinking-body${w_?" agent-thinking-body-collapsible":""}`,P0=w_?`--agent-thinking-collapsed-lines: ${M_};`:"";return B`
        ...
        <div class="agent-status-panel">
            ...
            ${y&&y_({panelTitle:N_("Thoughts"),text:A.text,fullText:A.fullText,totalLines:A.totalLines,maxLines:8,titleClass:"thought",panelKey:"thought"})}
    ...
    `}function Q6({src:_,onClose:$}){return S(()=>{let j=(Z)=>{if(Z.key==="Escape")$()};return document.addEventListener("keydown",j),()=>document.removeEventListener("keydown",j)},[$]),B`
        <div class="image-modal" onClick=${$}>
            <img src=${_} alt="Full size" />
        </div>
    `}function r7({mediaId:_,onPreview:$}){let[j,Z]=v(null);if(S(()=>{k2(_).then(Z).catch(()=>{})},[_]),!j)return null;let N=j.filename||"file",G=j.metadata?.size,Y=G?L$(G):"",L=$1(j.content_type,j.filename)==="unsupported"?"Details":"Preview";return B`
    ...
    `}function a7({attachment:_,onPreview:$}){let j=Number(_?.id),[Z,N]=v(null);S(()=>{if(!Number.isFinite(j))return;k2(j).then(N).catch(()=>{});return},[j]);let G=Z?.filename||_.label||`attachment-${_.id}`,Y=Number.isFinite(j)?V$(j):null,L=$1(Z?.content_type,Z?.filename||_?.label)==="unsupported"?"Details":"Preview";return B`
...
`).trim(),{content:L,attachments:N}}function Lj(_){return _.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}function Qj(_,$){if(!_||!$)return _;let j=String($).trim().split(/\s+/).filter(Boolean);if(j.length===0)return _;let Z=j.map(Lj).sort((Q,V)=>V.length-Q.length),N=new RegExp(`(${Z.join("|")})`,"gi"),G=new RegExp(`^(${Z.join("|")})$`,"i"),Y=new DOMParser().parseFromString(_,"text/html"),q=Y.createTreeWalker(Y.body,NodeFilter.SHOW_TEXT),L=[],X;while(X=q.nextNode())L.push(X);for(let Q of L){let V=Q.nodeValue;if(!V||!N.test(V)){N.lastIndex=0;continue}N.lastIndex=0;let H=Q.parentElement;if(H&&H.closest("code, pre, script, style"))continue;let P=V.split(N).filter((m)=>m!=="");if(P.length===0)continue;let f=Y.createDocumentFragment();for(let m of P)if(G.test(m)){let g=Y.createElement("mark");g.className="search-highlight-term",g.textContent=m,f.appendChild(g)}else f.appendChild(Y.createTextNode(m));Q.parentNode.replaceChild(f,Q)}return Y.body.innerHTML}function B6({post:_,onClick:$,onHashtagClick:j,onMessageRef:Z,onScrollToMessage:N,agentName:G,agentAvatarUrl:Y,userName:q,userAvatarUrl:L,userAvatarBackground:X,onDelete:Q,isThreadReply:V,isThreadPrev:H,isThreadNext:P,isRemoving:f,highlightQuery:m,onFileRef:g,onOpenWidget:A}){let[M,c]=v(null),y=k(null),R=_.data,n=R.type==="agent_response",e=q||"You",o=n?G||o8:e,j_=n?T4(G,Y,!0):T4(e,L),i=typeof X==="string"?X.trim().toLowerCase():"",X_=!n&&j_.image&&(i==="clear"||i==="transparent"),d_=n&&Boolean(j_.image),g_=`background-color: ${X_||d_?"transparent":j_.color}`,d=R.content_meta,N_=Boolean(d?.truncated),__=Boolean(d?.preview),V_=N_&&!__,Y_=N_?{originalLength:Number.isFinite(d?.original_length)?d.original_length:R.content?R.content.length:0,maxLength:Number.isFinite(d?.max_length)?d.max_length:0}:null,L_=R.content_blocks||[],B_=R.media_ids||[],G_=Zj(R.content,R.link_previews),{content:u_,fileRefs:I_}=qj(G_),{content:y_,messageRefs:E_}=Xj(u_),{content:z0,attachments:K0}=Vj(y_);G_=z0;let W_=v4(L_),$_=u4(L_),F_=W_.length===1&&typeof W_[0]?.fallback_text==="string"?W_[0].fallback_text.trim():"",Z_=$_.length===1?X6($_[0]).trim():"",H_=Boolean(F_)&&G_?.trim()===F_||Boolean(Z_)&&G_?.trim()===Z_,M_=Boolean(G_)&&!V_&&!H_,a_=typeof m==="string"?m.trim():"",p_=R_(()=>{if(!G_||H_)return"";let C=_$(G_,j);return a_?Qj(C,a_):C},[G_,H_,a_]),o_=(C,a)=>{C.stopPropagation(),c(V$(a))},[c_,h_]=v(null),w_=(C)=>{h_(C)},k_=(C)=>{C.stopPropagation(),Q?.(_)},m_=(C,a)=>{let U_=new Set;if(!C||a.length===0)return{content:C,usedIds:U_};return{content:C.replace(/attachment:([^\s)"']+)/g,(s_,F0,f_,S0)=>{let $$=F0.replace(/^\/+/,""),b0=a.find((H0)=>H0.name&&H0.name.toLowerCase()===$$.toLowerCase()&&!U_.has(H0.id))||a.find((H0)=>!U_.has(H0.id));if(!b0)return s_;if(U_.add(b0.id),S0.slice(Math.max(0,f_-2),f_)==="](")return`/media/${b0.id}`;return b0.name||"attachment"}),usedIds:U_}},W0=[],P0=[],b_=[],y0=[],j0=[],C_=[],Z0=[],t_=0;if(L_.length>0)L_.forEach((C)=>{if(C?.type==="text"&&C.annotations)Z0.push(C.annotations);if(C?.type==="generated_widget")C_.push(C);else if(C?.type==="resource_link")y0.push(C);else if(C?.type==="resource")j0.push(C);else if(C?.type==="file"){let a=B_[t_++];if(a)P0.push(a),b_.push({id:a,name:C?.name||C?.filename||C?.title})}else if(C?.type==="image"||!C?.type){let a=B_[t_++];if(a){let U_=typeof C?.mime_type==="string"?C.mime_type:void 0;W0.push({id:a,annotations:C?.annotations,mimeType:U_}),b_.push({id:a,name:C?.name||C?.filename||C?.title})}}});else if(B_.length>0){let C=K0.length>0;B_.forEach((a,U_)=>{let x_=K0[U_]||null;if(b_.push({id:a,name:x_?.label||null}),C)P0.push(a);else W0.push({id:a,annotations:null})})}if(K0.length>0)K0.forEach((C)=>{if(!C?.id)return;let a=b_.find((U_)=>String(U_.id)===String(C.id));if(a&&!a.name)a.name=C.label});let{content:$0,usedIds:N0}=m_(G_,b_);G_=$0;let T_=W0.filter(({id:C})=>!N0.has(C)),O0=P0.filter((C)=>!N0.has(C)),T0=K0.length>0?K0.map((C,a)=>({id:C.id||`attachment-${a+1}`,label:C.label||`attachment-${a+1}`})):b_.map((C,a)=>({id:C.id,label:C.name||`attachment-${a+1}`})),A_=R_(()=>v4(L_),[L_]),e_=R_(()=>u4(L_),[L_]),f0=R_(()=>{return A_.map((C)=>`${C.card_id}:${C.state}`).join("|")},[A_]);S(()=>{if(!y.current)return;return v$(y.current),Wj(y.current)},[p_]);let P_=k(null);return S(()=>{if(!P_.current||A_.length===0)return;let C=P_.current;C.innerHTML="";for(let a of A_){let U_=document.createElement("div");C.appendChild(U_),W6(U_,a,{onAction:async(x_)=>{if(x_.type==="Action.OpenUrl"){let s_=q2(x_.url||"");if(!s_)throw Error("Invalid URL");window.open(s_,"_blank","noopener,noreferrer");return}if(x_.type==="Action.Submit"){await X4({post_id:_.id,thread_id:R.thread_id||_.id,chat_jid:_.chat_jid||null,card_id:a.card_id,action:{type:x_.type,title:x_.title||"",data:x_.data}});return}console.warn("[post] unsupported adaptive card action:",x_.type,x_)}}).catch((x_)=>{console.error("[post] adaptive card render error:",x_),U_.textContent=a.fallback_text||"Card failed to render."})}},[f0,_.id]),B`
        ...
        ${c_&&B`
            ...
            />
        `}
    `}function U6({posts:_,hasMore:$,onLoadMore:j,onPostClick:Z,onHashtagClick:N,onMessageRef:G,onScrollToMessage:Y,onFileRef:q,onOpenWidget:L,emptyMessage:X,timelineRef:Q,agents:V,user:H,onDeletePost:P,reverse:f=!0,removingPostIds:m,searchQuery:g}){let[A,M]=v(!1),c=k(null),y=typeof IntersectionObserver<"u",R=I(async()=>{if(!j||!$||A)return;M(!0);try{await j({preserveScroll:!0,preserveMode:"top"})}finally{M(!1)}},[$,A,j]),n=I((d)=>{let{scrollTop:N_,scrollHeight:__,clientHeight:V_}=d.target,Y_=f?__-V_-N_:N_,L_=Math.max(300,V_);if(Y_<L_)R()},[f,R]);S(()=>{if(!y)return;let d=c.current,N_=Q?.current;if(!d||!N_)return;let __=300,V_=new IntersectionObserver((Y_)=>{for(let L_ of Y_){if(!L_.isIntersecting)continue;R()}},{root:N_,rootMargin:`${__}px 0px ${__}px 0px`,threshold:0});return V_.observe(d),()=>V_.disconnect()},[y,$,j,Q,R]);let e=k(R);if(e.current=R,S(()=>{if(y)return;if(!Q?.current)return;let{scrollTop:d,scrollHeight:N_,clientHeight:__}=Q.current,V_=f?N_-__-d:d,Y_=Math.max(300,__);if(V_<Y_)e.current?.()},[y,_,$,f,Q]),S(()=>{if(!Q?.current)return;if(!$||A)return;let{scrollTop:d,scrollHeight:N_,clientHeight:__}=Q.current,V_=f?N_-__-d:d,Y_=Math.max(300,__);if(N_<=__+1||V_<Y_)e.current?.()},[_,$,A,f,Q]),!_)return B`<div class="loading"><div class="spinner"></div></div>`;if(_.length===0)return B`
...
`),this.setStatus("Unavailable")}}sendResize(){if(!this.socket||this.socket.readyState!==WebSocket.OPEN||!this.fitAddon?.proposeDimensions)return;let _=this.fitAddon.proposeDimensions();if(!_)return;this.socket.send(JSON.stringify({type:"resize",cols:_.cols,rows:_.rows}))}getContent(){return}isDirty(){return!1}focus(){if(this.terminal?.focus){this.terminal.focus();return}this.termEl?.focus()}resize(){this.syncHostLayout();try{this.terminal?.renderer?.remeasureFont?.()}catch{}try{this.fitAddon?.fit?.()}catch{}try{this.terminal?.refresh?.()}catch{}this.syncHostLayout(),this.sendResize()}dispose(){if(this.disposed)return;this.disposed=!0;try{if(this.resizeFrame)cancelAnimationFrame(this.resizeFrame),this.resizeFrame=0}catch{}try{if(this.themeChangeListener)window.removeEventListener("piclaw-theme-change",this.themeChangeListener)}catch{}try{if(this.mediaQuery&&this.mediaQueryListener){if(this.mediaQuery.removeEventListener)this.mediaQuery.removeEventListener("change",this.mediaQueryListener);else if(this.mediaQuery.removeListener)this.mediaQuery.removeListener(this.mediaQueryListener)}}catch{}try{if(this.dockResizeListener)window.removeEventListener("dock-resize",this.dockResizeListener);if(this.windowResizeListener)window.removeEventListener("resize",this.windowResizeListener)}catch{}try{this.themeObserver?.disconnect?.()}catch{}try{this.resizeObserver?.disconnect?.()}catch{}try{this.socket?.close?.()}catch{}try{this.fitAddon?.dispose?.()}catch{}try{this.terminal?.dispose?.()}catch{}this.termEl?.remove()}}var l4={id:"terminal",label:"Terminal",icon:"terminal",capabilities:["terminal"],placement:"dock",mount(_,$){return new i4(_,$)}},n4={id:"terminal-tab",label:"Terminal",icon:"terminal",capabilities:["terminal"],placement:"tabs",canHandle(_){return _?.path==="piclaw://terminal"?1e4:!1},mount(_,$){return new i4(_,$)}};function s$(_){return String(_||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function wj(_,$){let j=String(_||"").trim();if(!j)return j;if(/^[a-zA-Z][a-zA-Z\d+.-]*:/.test(j)||j.startsWith("#")||j.startsWith("data:")||j.startsWith("blob:"))return j;let Z=j.match(/^([^?#]*)(\?[^#]*)?(#.*)?$/),N=Z?.[1]||j,G=Z?.[2]||"",Y=Z?.[3]||"",q=String($||"").split("/").slice(0,-1).join("/"),X=N.startsWith("/")?N:`${q?`${q}/`:""}${N}`,Q=[];for(let H of X.split("/")){if(!H||H===".")continue;if(H===".."){if(Q.length>0)Q.pop();continue}Q.push(H)}let V=Q.join("/");return`${D1(V)}${G}${Y}`}function Z1(_){return _?.preview||null}function Pj(_){let $=String(_||""),j=Math.max($.lastIndexOf("/"),$.lastIndexOf("\\")),Z=j>=0?$.slice(j+1):$,N=Z.lastIndexOf(".");if(N<=0||N===Z.length-1)return"none";return Z.slice(N+1)}function xj(_){if(!_)return"unknown";if(_.kind==="image")return"image";if(_.kind==="text")return _.content_type==="text/markdown"?"markdown":"text";if(_.kind==="binary")return"binary";return String(_.kind||"unknown")}function Ij(_,$){let j=$?.path||_?.path||"",Z=[];if($?.content_type)Z.push(`<span><strong>type:</strong> ${s$($.content_type)}</span>`);if(typeof $?.size==="number")Z.push(`<span><strong>size:</strong> ${s$(L$($.size))}</span>`);if($?.mtime)Z.push(`<span><strong>modified:</strong> ${s$(X2($.mtime))}</span>`);if(Z.push(`<span><strong>kind:</strong> ${s$(xj($))}</span>`),Z.push(`<span><strong>extension:</strong> ${s$(Pj(j))}</span>`),j)Z.push(`<span><strong>path:</strong> ${s$(j)}</span>`);if($?.truncated)Z.push("<span><strong>content:</strong> truncated</span>");return`<div class="workspace-preview-meta workspace-preview-meta-inline">${Z.join("")}</div>`}function Mj(_){let $=Z1(_);if(!$)return'<div class="workspace-preview-text">No preview available.</div>';let j=Ij(_,$);if($.kind==="image"){let Z=$.url||($.path?D1($.path):"");return`${j}
    ...
    `}function _Z({mediaId:_}){let[$,j]=v(null);if(S(()=>{if(!_)return;k2(_).then(j).catch(()=>{})},[_]),!$)return null;let Z=$.filename||"file",N=$.metadata?.size?L$($.metadata.size):"";return B`
    ...
    `}var $Z=/\.(docx?|xlsx?|pptx?|odt|ods|odp|rtf)$/i,jZ=/\.(csv|tsv)$/i,ZZ=/\.pdf$/i,NZ=/\.(png|jpe?g|gif|webp|bmp|ico|svg)$/i,_9=/\.drawio(\.xml|\.svg|\.png)?$/i;function $9({tabs:_,activeId:$,onActivate:j,onClose:Z,onCloseOthers:N,onCloseAll:G,onTogglePin:Y,onTogglePreview:q,previewTabs:L,onToggleDock:X,dockVisible:Q,onToggleZen:V,zenMode:H}){let[P,f]=v(null),m=k(null);S(()=>{if(!P)return;let y=(R)=>{if(R.type==="keydown"&&R.key!=="Escape")return;f(null)};return document.addEventListener("click",y),document.addEventListener("keydown",y),()=>{document.removeEventListener("click",y),document.removeEventListener("keydown",y)}},[P]),S(()=>{let y=(R)=>{if(R.ctrlKey&&R.key==="Tab"){if(R.preventDefault(),!_.length)return;let n=_.findIndex((e)=>e.id===$);if(R.shiftKey){let e=_[(n-1+_.length)%_.length];j?.(e.id)}else{let e=_[(n+1)%_.length];j?.(e.id)}return}if((R.ctrlKey||R.metaKey)&&R.key==="w"){let n=document.querySelector(".editor-pane");if(n&&n.contains(document.activeElement)){if(R.preventDefault(),$)Z?.($)}}};return document.addEventListener("keydown",y),()=>document.removeEventListener("keydown",y)},[_,$,j,Z]);let g=I((y,R)=>{if(y.button===1){y.preventDefault(),Z?.(R);return}if(y.button===0)j?.(R)},[j,Z]),A=I((y,R)=>{y.preventDefault(),f({id:R,x:y.clientX,y:y.clientY})},[]),M=I((y)=>{y.preventDefault(),y.stopPropagation()},[]),c=I((y,R)=>{y.preventDefault(),y.stopPropagation(),Z?.(R)},[Z]);if(S(()=>{if(!$||!m.current)return;let y=m.current.querySelector(".tab-item.active");if(y)y.scrollIntoView({block:"nearest",inline:"nearest",behavior:"smooth"})},[$]),!_.length)return null;return B`
        <div class="tab-strip" ref=${m} role="tablist">
            ${_.map((y)=>B`
                <div
                    ...
                    aria-selected=${y.id===$}
                    title=${y.path}
                    onMouseDown=${(R)=>g(R,y.id)}
                    onContextMenu=${(R)=>A(R,y.id)}
                >
                    ${y.pinned&&B`
        ...
        ${P&&B`
            <div class="tab-context-menu" style=${{left:P.x+"px",top:P.y+"px"}}>
                ...
                ${$Z.test(P.id)&&B`
                    <hr />
                    <button onClick=${()=>{let y="/workspace/raw?path="+encodeURIComponent(P.id),R=P.id.split("/").pop()||"document",n="/office-viewer/?url="+encodeURIComponent(y)+"&name="+encodeURIComponent(R);window.open(n,"_blank","noopener"),f(null)}}>Open in New Tab</button>
                `}
                ${jZ.test(P.id)&&B`
                    <hr />
        ...
        `}
    `}var GZ=400,X3=60,j9=220,V3="mdPreviewHeight";function YZ(){try{let _=localStorage.getItem(V3),$=_?Number(_):NaN;return Number.isFinite($)&&$>=X3?$:j9}catch{return j9}}function Z9({getContent:_,path:$,onClose:j}){let[Z,N]=v(""),[G,Y]=v(YZ),q=k(null),L=k(null),X=k(""),Q=k(_);return Q.current=_,S(()=>{let P=()=>{let m=Q.current?.()||"";if(m===X.current)return;X.current=m;try{let g=_$(m,null,{sanitize:!1});N(g)}catch{N('<p style="color:var(--text-secondary)">Preview unavailable</p>')}};P();let f=setInterval(P,GZ);return()=>clearInterval(f)},[]),S(()=>{if(q.current&&Z)v$(q.current).catch(()=>{})},[Z]),B`
        <div
            class="md-preview-splitter"
            onMouseDown=${(P)=>{P.preventDefault();let f=P.clientY,m=L.current?.offsetHeight||G,g=L.current?.parentElement,A=g?g.offsetHeight*0.7:500,M=P.currentTarget;M.classList.add("dragging"),document.body.style.cursor="row-resize",document.body.style.userSelect="none";let c=(R)=>{let n=Math.min(Math.max(m-(R.clientY-f),X3),A);Y(n)},y=()=>{M.classList.remove("dragging"),document.body.style.cursor="",document.body.style.userSelect="";try{localStorage.setItem(V3,String(Math.round(L.current?.offsetHeight||G)))}catch{}document.removeEventListener("mousemove",c),document.removeEventListener("mouseup",y)};document.addEventListener("mousemove",c),document.addEventListener("mouseup",y)}}
            onTouchStart=${(P)=>{P.preventDefault();let f=P.touches[0];if(!f)return;let m=f.clientY,g=L.current?.offsetHeight||G,A=L.current?.parentElement,M=A?A.offsetHeight*0.7:500,c=P.currentTarget;c.classList.add("dragging"),document.body.style.userSelect="none";let y=(n)=>{let e=n.touches[0];if(!e)return;n.preventDefault();let o=Math.min(Math.max(g-(e.clientY-m),X3),M);Y(o)},R=()=>{c.classList.remove("dragging"),document.body.style.userSelect="";try{localStorage.setItem(V3,String(Math.round(L.current?.offsetHeight||G)))}catch{}document.removeEventListener("touchmove",y),document.removeEventListener("touchend",R),document.removeEventListener("touchcancel",R)};document.addEventListener("touchmove",y,{passive:!1}),document.addEventListener("touchend",R),document.addEventListener("touchcancel",R)}}
        ></div>
    ...
    `}function N9({handleSseEvent:_,handleConnectionStatusChange:$,loadPosts:j,onWake:Z,chatJid:N}){let G=k(_);G.current=_;let Y=k($);Y.current=$;let q=k(j);q.current=j;let L=k(Z);L.current=Z,S(()=>{q.current();let X=new y1((V,H)=>G.current(V,H),(V)=>Y.current(V),{chatJid:N});X.connect();let Q=()=>{X.reconnectIfNeeded();let V=typeof document<"u"?document:null;if(!V||V.visibilityState==="visible")L.current?.()};return window.addEventListener("focus",Q),document.addEventListener("visibilitychange",Q),()=>{window.removeEventListener("focus",Q),document.removeEventListener("visibilitychange",Q),X.disconnect()}},[N])}function G9(){let[_,$]=v(!1),[j,Z]=v("default"),N=k(!1);S(()=>{let L=A2("notificationsEnabled",!1);if(N.current=L,$(L),typeof Notification<"u")Z(Notification.permission)},[]),S(()=>{N.current=_},[_]);let G=I(()=>{if(typeof Notification>"u")return Promise.resolve("denied");try{let L=Notification.requestPermission();if(L&&typeof L.then==="function")return L;return Promise.resolve(L)}catch{return Promise.resolve("default")}},[]),Y=I(async()=>{if(typeof window>"u"||typeof Notification>"u")return;if(!window.isSecureContext){alert("Notifications require a secure context (HTTPS or installed app).");return}if(Notification.permission==="denied"){Z("denied"),alert("Browser notifications are blocked. Enable them in your browser settings.");return}if(Notification.permission==="default"){let X=await G();if(Z(X||"default"),X!=="granted"){N.current=!1,$(!1),l_("notificationsEnabled","false");return}}let L=!N.current;N.current=L,$(L),l_("notificationsEnabled",String(L))},[G]),q=I((L,X)=>{if(!N.current)return!1;if(typeof Notification>"u")return!1;if(Notification.permission!=="granted")return!1;try{let Q=new Notification(L,{body:X});return Q.onclick=()=>{try{window.focus()}catch{}},!0}catch{return!1}},[]);return{notificationsEnabled:_,notificationPermission:j,toggleNotifications:Y,notify:q}}var N1=(_)=>{let $=new Set;return(_||[]).filter((j)=>{if(!j||$.has(j.id))return!1;return $.add(j.id),!0})};function Y9({preserveTimelineScroll:_,preserveTimelineScrollTop:$,chatJid:j=null}){let[Z,N]=v(null),[G,Y]=v(!1),q=k(!1),L=k(null),X=k(!1),Q=k(null),V=k(null),H=k(0);S(()=>{q.current=G},[G]),S(()=>{V.current=Z},[Z]),S(()=>{H.current+=1,V.current=null,Q.current=null,X.current=!1,q.current=!1,N(null),Y(!1)},[j]);let P=I(async(g=null)=>{let A=H.current;try{if(g){let M=await Z4(g,50,0,j);if(A!==H.current)return;N(M.posts),Y(!1)}else{let M=await l2(10,null,j);if(A!==H.current)return;N(M.posts),Y(M.has_more)}}catch(M){if(A!==H.current)return;console.error("Failed to load posts:",M)}},[j]),f=I(async()=>{let g=H.current;try{let A=await l2(10,null,j);if(g!==H.current)return;N((M)=>{if(!M||M.length===0)return A.posts;return N1([...A.posts,...M])}),Y((M)=>M||A.has_more)}catch(A){if(g!==H.current)return;console.error("Failed to refresh timeline:",A)}},[j]),m=I(async(g={})=>{let A=H.current,M=V.current;if(!M||M.length===0)return;if(X.current)return;let{preserveScroll:c=!0,preserveMode:y="top",allowRepeat:R=!1}=g,n=(j_)=>{if(!c){j_();return}if(y==="top")$(j_);else _(j_)},o=M.slice().sort((j_,i)=>j_.id-i.id)[0]?.id;if(!Number.isFinite(o))return;if(!R&&Q.current===o)return;X.current=!0,Q.current=o;try{let j_=await l2(10,o,j);if(A!==H.current)return;if(j_.posts.length>0)n(()=>{N((i)=>N1([...j_.posts,...i||[]])),Y(j_.has_more)});else Y(!1)}catch(j_){if(A!==H.current)return;console.error("Failed to load more posts:",j_)}finally{if(A===H.current)X.current=!1}},[j,_,$]);return S(()=>{L.current=m},[m]),{posts:Z,setPosts:N,hasMore:G,setHasMore:Y,hasMoreRef:q,loadPosts:P,refreshTimeline:f,loadMore:m,loadMoreRef:L,loadingMoreRef:X,lastBeforeIdRef:Q}}function z9(){let[_,$]=v(null),[j,Z]=v({text:"",totalLines:0}),[N,G]=v(""),[Y,q]=v({text:"",totalLines:0}),[L,X]=v(null),[Q,V]=v(null),[H,P]=v(null),f=k(null),m=k(0),g=k(!1),A=k(""),M=k(""),c=k(null),y=k(null),R=k(null),n=k(null),e=k(!1),o=k(!1);return{agentStatus:_,setAgentStatus:$,agentDraft:j,setAgentDraft:Z,agentPlan:N,setAgentPlan:G,agentThought:Y,setAgentThought:q,pendingRequest:L,setPendingRequest:X,currentTurnId:Q,setCurrentTurnId:V,steerQueuedTurnId:H,setSteerQueuedTurnId:P,lastAgentEventRef:f,lastSilenceNoticeRef:m,isAgentRunningRef:g,draftBufferRef:A,thoughtBufferRef:M,pendingRequestRef:c,stalledPostIdRef:y,currentTurnIdRef:R,steerQueuedTurnIdRef:n,thoughtExpandedRef:e,draftExpandedRef:o}}function K9({appShellRef:_,sidebarWidthRef:$,editorWidthRef:j,dockHeightRef:Z}){let N=k((Q)=>{Q.preventDefault();let V=_.current;if(!V)return;let H=Q.clientX,P=$.current||280,f=Q.currentTarget;f.classList.add("dragging"),V.classList.add("sidebar-resizing"),document.body.style.cursor="col-resize",document.body.style.userSelect="none";let m=H,g=(M)=>{m=M.clientX;let c=Math.min(Math.max(P+(M.clientX-H),160),600);V.style.setProperty("--sidebar-width",`${c}px`),$.current=c},A=()=>{let M=Math.min(Math.max(P+(m-H),160),600);$.current=M,f.classList.remove("dragging"),V.classList.remove("sidebar-resizing"),document.body.style.cursor="",document.body.style.userSelect="",l_("sidebarWidth",String(Math.round(M))),document.removeEventListener("mousemove",g),document.removeEventListener("mouseup",A)};document.addEventListener("mousemove",g),document.addEventListener("mouseup",A)}).current,G=k((Q)=>{Q.preventDefault();let V=_.current;if(!V)return;let H=Q.touches[0];if(!H)return;let P=H.clientX,f=$.current||280,m=Q.currentTarget;m.classList.add("dragging"),V.classList.add("sidebar-resizing"),document.body.style.userSelect="none";let g=(M)=>{let c=M.touches[0];if(!c)return;M.preventDefault();let y=Math.min(Math.max(f+(c.clientX-P),160),600);V.style.setProperty("--sidebar-width",`${y}px`),$.current=y},A=()=>{m.classList.remove("dragging"),V.classList.remove("sidebar-resizing"),document.body.style.userSelect="",l_("sidebarWidth",String(Math.round($.current||f))),document.removeEventListener("touchmove",g),document.removeEventListener("touchend",A),document.removeEventListener("touchcancel",A)};document.addEventListener("touchmove",g,{passive:!1}),document.addEventListener("touchend",A),document.addEventListener("touchcancel",A)}).current,Y=k((Q)=>{Q.preventDefault();let V=_.current;if(!V)return;let H=Q.clientX,P=j.current||$.current||280,f=Q.currentTarget;f.classList.add("dragging"),document.body.style.cursor="col-resize",document.body.style.userSelect="none";let m=H,g=(M)=>{m=M.clientX;let c=Math.min(Math.max(P+(M.clientX-H),200),800);V.style.setProperty("--editor-width",`${c}px`),j.current=c},A=()=>{let M=Math.min(Math.max(P+(m-H),200),800);j.current=M,f.classList.remove("dragging"),document.body.style.cursor="",document.body.style.userSelect="",l_("editorWidth",String(Math.round(M))),document.removeEventListener("mousemove",g),document.removeEventListener("mouseup",A)};document.addEventListener("mousemove",g),document.addEventListener("mouseup",A)}).current,q=k((Q)=>{Q.preventDefault();let V=_.current;if(!V)return;let H=Q.touches[0];if(!H)return;let P=H.clientX,f=j.current||$.current||280,m=Q.currentTarget;m.classList.add("dragging"),document.body.style.userSelect="none";let g=(M)=>{let c=M.touches[0];if(!c)return;M.preventDefault();let y=Math.min(Math.max(f+(c.clientX-P),200),800);V.style.setProperty("--editor-width",`${y}px`),j.current=y},A=()=>{m.classList.remove("dragging"),document.body.style.userSelect="",l_("editorWidth",String(Math.round(j.current||f))),document.removeEventListener("touchmove",g),document.removeEventListener("touchend",A),document.removeEventListener("touchcancel",A)};document.addEventListener("touchmove",g,{passive:!1}),document.addEventListener("touchend",A),document.addEventListener("touchcancel",A)}).current,L=k((Q)=>{Q.preventDefault();let V=_.current;if(!V)return;let H=Q.clientY,P=Z?.current||200,f=Q.currentTarget;f.classList.add("dragging"),document.body.style.cursor="row-resize",document.body.style.userSelect="none";let m=H,g=(M)=>{m=M.clientY;let c=Math.min(Math.max(P-(M.clientY-H),100),window.innerHeight*0.5);if(V.style.setProperty("--dock-height",`${c}px`),Z)Z.current=c;window.dispatchEvent(new CustomEvent("dock-resize"))},A=()=>{let M=Math.min(Math.max(P-(m-H),100),window.innerHeight*0.5);if(Z)Z.current=M;f.classList.remove("dragging"),document.body.style.cursor="",document.body.style.userSelect="",l_("dockHeight",String(Math.round(M))),window.dispatchEvent(new CustomEvent("dock-resize")),document.removeEventListener("mousemove",g),document.removeEventListener("mouseup",A)};document.addEventListener("mousemove",g),document.addEventListener("mouseup",A)}).current,X=k((Q)=>{Q.preventDefault();let V=_.current;if(!V)return;let H=Q.touches[0];if(!H)return;let P=H.clientY,f=Z?.current||200,m=Q.currentTarget;m.classList.add("dragging"),document.body.style.userSelect="none";let g=(M)=>{let c=M.touches[0];if(!c)return;M.preventDefault();let y=Math.min(Math.max(f-(c.clientY-P),100),window.innerHeight*0.5);if(V.style.setProperty("--dock-height",`${y}px`),Z)Z.current=y;window.dispatchEvent(new CustomEvent("dock-resize"))},A=()=>{m.classList.remove("dragging"),document.body.style.userSelect="",l_("dockHeight",String(Math.round(Z?.current||f))),window.dispatchEvent(new CustomEvent("dock-resize")),document.removeEventListener("touchmove",g),document.removeEventListener("touchend",A),document.removeEventListener("touchcancel",A)};document.addEventListener("touchmove",g,{passive:!1}),document.addEventListener("touchend",A),document.addEventListener("touchcancel",A)}).current;return{handleSplitterMouseDown:N,handleSplitterTouchStart:G,handleEditorSplitterMouseDown:Y,handleEditorSplitterTouchStart:q,handleDockSplitterMouseDown:L,handleDockSplitterTouchStart:X}}function W9({onTabClosed:_}={}){let $=k(_);$.current=_;let[j,Z]=v(()=>n_.getTabs()),[N,G]=v(()=>n_.getActiveId()),[Y,q]=v(()=>n_.getTabs().length>0);S(()=>{return n_.onChange((y,R)=>{Z(y),G(R),q(y.length>0)})},[]);let[L,X]=v(()=>new Set),Q=I((y)=>{X((R)=>{let n=new Set(R);if(n.has(y))n.delete(y);else n.add(y);return n})},[]),V=I((y)=>{X((R)=>{if(!R.has(y))return R;let n=new Set(R);return n.delete(y),n})},[]),H=I((y,R={})=>{if(!y)return;let n={path:y,mode:"edit"};try{if(!r_.resolve(n)){if(!r_.get("editor")){console.warn(`[openEditor] No pane handler for: ${y}`);return}}}catch(o){console.warn(`[openEditor] paneRegistry.resolve() error for "${y}":`,o)}let e=typeof R?.label==="string"&&R.label.trim()?R.label.trim():void 0;n_.open(y,e)},[]),P=I(()=>{let y=n_.getActiveId();if(y){let R=n_.get(y);if(R?.dirty){if(!window.confirm(`"${R.label}" has unsaved changes. Close anyway?`))return}n_.close(y),V(y),$.current?.(y)}},[V]),f=I((y)=>{let R=n_.get(y);if(R?.dirty){if(!window.confirm(`"${R.label}" has unsaved changes. Close anyway?`))return}n_.close(y),V(y),$.current?.(y)},[V]),m=I((y)=>{n_.activate(y)},[]),g=I((y)=>{let R=n_.getTabs().filter((o)=>o.id!==y&&!o.pinned),n=R.filter((o)=>o.dirty).length;if(n>0){if(!window.confirm(`${n} unsaved tab${n>1?"s":""} will be closed. Continue?`))return}let e=R.map((o)=>o.id);n_.closeOthers(y),e.forEach((o)=>{V(o),$.current?.(o)})},[V]),A=I(()=>{let y=n_.getTabs().filter((e)=>!e.pinned),R=y.filter((e)=>e.dirty).length;if(R>0){if(!window.confirm(`${R} unsaved tab${R>1?"s":""} will be closed. Continue?`))return}let n=y.map((e)=>e.id);n_.closeAll(),n.forEach((e)=>{V(e),$.current?.(e)})},[V]),M=I((y)=>{n_.togglePin(y)},[]),c=I(()=>{let y=n_.getActiveId();if(y)window.dispatchEvent(new CustomEvent("workspace-reveal-path",{detail:{path:y}}))},[]);return S(()=>{let y=(R)=>{let{oldPath:n,newPath:e,type:o}=R.detail||{};if(!n||!e)return;if(o==="dir"){for(let j_ of n_.getTabs())if(j_.path===n||j_.path.startsWith(`${n}/`)){let i=`${e}${j_.path.slice(n.length)}`;n_.rename(j_.id,i)}}else n_.rename(n,e)};return window.addEventListener("workspace-file-renamed",y),()=>window.removeEventListener("workspace-file-renamed",y)},[]),S(()=>{let y=(R)=>{if(n_.hasUnsaved())R.preventDefault(),R.returnValue=""};return window.addEventListener("beforeunload",y),()=>window.removeEventListener("beforeunload",y)},[]),{editorOpen:Y,tabStripTabs:j,tabStripActiveId:N,previewTabs:L,openEditor:H,closeEditor:P,handleTabClose:f,handleTabActivate:m,handleTabCloseOthers:g,handleTabCloseAll:A,handleTabTogglePin:M,handleTabTogglePreview:Q,revealInExplorer:c}}function L3(_,$){try{if(typeof window>"u")return $;let j=window.__PICLAW_SILENCE||{},Z=`__PICLAW_SILENCE_${_.toUpperCase()}_MS`,N=j[_]??window[Z],G=Number(N);return Number.isFinite(G)?G:$}catch{return $}}var Q3=L3("warning",30000),q9=L3("finalize",120000),B3=L3("refresh",30000),X9=30000;function V9(_){let $={};return(_?.agents||[]).forEach((j)=>{$[j.id]=j}),$}function L9(){if(/iPad|iPhone/.test(navigator.userAgent))return!0;return navigator.platform==="MacIntel"&&navigator.maxTouchPoints>1}function Q9(_=30000){let[,$]=v(0);S(()=>{let j=setInterval(()=>$((Z)=>Z+1),_);return()=>clearInterval(j)},[_])}function U3(_,$=160){let j=String(_||"").replace(/\r\n/g,`
...
//# debugId=33D8ACEE580B877E64756E2164756E21
