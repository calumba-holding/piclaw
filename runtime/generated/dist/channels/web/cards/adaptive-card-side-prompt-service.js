import { DEFAULT_WEB_USER_ID, createWebSession, deleteAllWebSessions, getMessageByAnyRowId, getMessageByRowId, replaceMessageContent, } from "../../../db.js";
import { setWebTotpSecret } from "../../../core/config.js";
import { createLogger } from "../../../utils/logger.js";
import { buildAdaptiveCardSubmissionText, buildAdaptiveCardSubmitBlock, getAdaptiveCardSubmitBehavior, getAdaptiveCardSubmitState, getAdaptiveCardTestFailure, markAdaptiveCardState, sanitizeAdaptiveCardActionPayload, sanitizeAdaptiveCardSubmissionData, } from "./adaptive-card-actions.js";
...
function parseSidePromptPayload(payload, defaultChatJid) {
    return {
...
export function createWebAdaptiveCardSidePromptService(channel, options) {
    return new WebAdaptiveCardSidePromptService({
        ...
        skipFailedOnModelSwitch: typeof channel.skipFailedOnModelSwitch === "function"
            ...
            : () => { },
        forwardAgentMessage: async (req, pathname, chatJid, agentId) => handleAgentMessageRequest(channel, req, pathname, chatJid, agentId),
    });
}
    ...
    async handleAdaptiveCardAction(req) {
        ...
        if (isLoginFlow) {
            ...
            if (authResult.status === "success" && authResult.model_label) {
                let nextModel = authResult.model_label ?? null;
                let thinkingLevel = authResult.thinking_level ?? null;
        ...
        const forwardRes = await this.options.forwardAgentMessage(forwardReq, `/agent/${this.options.defaultAgentId}/message`, chatJid, this.options.defaultAgentId);
        if (!forwardRes.ok) {
            return forwardRes;
        }
