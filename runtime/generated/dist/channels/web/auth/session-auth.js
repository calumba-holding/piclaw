...
export function buildSessionCookieHeader(token, req, sessionTtl, tlsConfigured) {
...
export function isRequestAuthenticated(req, authEnabled) {
    if (!authEnabled)
        return true;
...
export function isRequestTotpSession(req, totpEnabled) {
    if (!totpEnabled)
        return false;
    deleteExpiredWebSessions();
    const token = getSessionTokenFromRequest(req);
