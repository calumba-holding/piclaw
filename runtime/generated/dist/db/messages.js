 ...
 */
export function getMessageByAnyRowId(rowId) {
    const db = getDb();
    const row = db
        .prepare(`SELECT ${MESSAGE_COLUMNS} FROM messages WHERE rowid = ?`)
        .get(rowId);
    if (!row)
        return undefined;
    const mediaIds = getMediaIdsForMessage(row.rowid);
...
export function getTimeline(chatJid, limit, beforeId) {
    ...
    if (rowIds.length > 0) {
        ...
        for (const mediaRow of mediaRows) {
            ...
            if (current)
                current.push(mediaRow.media_id);
            else
                mediaByMessage.set(mediaRow.message_rowid, [mediaRow.media_id]);
        }
    }
