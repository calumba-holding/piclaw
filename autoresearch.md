...
- A small follow-up on the standalone keychain coverage test added deterministic coverage for invalid payload handling, invalid empty-name references, placeholder no-match behavior, and empty-post-delete listing; this pushed merged keychain coverage above 98% without widening runtime scope.
- Final reduction to zero uncovered lines came from a transparent source-aware normalizer in `./scripts/audit-core-config-keychain-coverage.sh`: Bun raw LCOV still reported blank lines, brace-only lines, and SQL/template-literal content rows as uncovered. The audit now records those exclusions explicitly in `artifacts/add-tests-core-config-and-keychain/coverage-exclusions.json` instead of silently treating them as executable misses.
